...
Time of Day offers full support for Unity 5 image-based ambient light and reflections. It can update both the per-scene ambient light and the per-scene reflection probe at runtime. Ambient light can be a solid color, a gradient or spherical harmonics. The reflection probe is always a cubemap. Both contain approximations of the atmosphere in the top half and lerp to the configured ambient light color towards the bottom half. This can be looked at as the ground color since Time of Day cannot know what color to use for the ground by itself.

In addition to the per-scene ambient light and the per-scene reflection probe, Unity 5 offers reflection probes that can be placed in a scene to capture reflections of their surroundings. Reflection probes can be refreshed in realtime or baked in the editor. Note that refreshing reflection probes is a fairly expensive operation and should be used with care. Howeever, baked reflection probes do not change over the course of the day / night cycle. It is therefore recommended to only use baked reflection probes inside buildings or in outdoor scenes with somewhat static lighting conditions. Unity automatically falls back to the dynamic per-scene reflections that are calculated by Time of Day for all objects that do not have reflection probes nearby.
