...
public partial class TOD_Sky : MonoBehaviour
{
	...
	private void SetupLightSource(float sun_theta, float sun_phi, float moon_theta, float moon_phi)
	{
		...
		float moon_phase  = (1-Mathf.Abs(Moon.Phase));
		...
		if (LerpValue > lerp_threshold)
		{
			...
			float light_a = Sun.LightColor.a;
		...
		else
		{
			...
			float lerp = clear_sky * moon_phase * moon_height * (lerp_threshold - LerpValue) / lerp_threshold;
			...
			float light_a = Moon.LightColor.a;
